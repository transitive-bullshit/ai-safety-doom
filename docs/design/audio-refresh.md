...
| Sam | Loaded server motor and two heavy contactors | A bass-heavy servo strike with a mechanical follow-through | Original Doom Baron death (`DSBRSDTH`) |

The four death recordings preserve their original 11,025 Hz, 8-bit mono PCM after removing DMX format padding. No pitch shifting, EQ, layering, or normalization changes the clips. Each successful death cue starts one buffer at its native rate, with positional attenuation/panning and a 0.18 room send through the existing compressor. Playback gain is 1.0 (1.1 for Sam) before attenuation. If a recording fails to load, only that enemy uses its previous synthesized death profile; a loaded sample replaces the synthesized cue completely. The three player vocal files, `playerVoice()` method, shotgun file, and both shotgun methods remain byte-for-byte unchanged. [Sources and extraction details](../../public/game/audio/README.md) describe the original Doom material and rights attribution.

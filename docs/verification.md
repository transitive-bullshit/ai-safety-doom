...
The former Baron WAV was valid and byte-for-byte identical to its original Doom lump; its processed 8-bit character was replaced rather than repaired. The new `doom-baron-death.wav` uses HaelDB's separate `3yell9` take, verified against the retained source archive, with a lower pitch and natural vocal tail. It is 1.78 seconds, mono 44.1 kHz 16-bit PCM, with peak −1.51 dBFS, RMS −9.59 dBFS, and no full-scale clipped samples. The other eight WAV hashes are unchanged.

Six Chrome before/after renders verify decoding, one complete Sam death source at rate 1 without looping, and shared compressor routing. Isolated output peaks at 0.465935; a dense 20-event overlap with score peaks at 0.881216, retaining 1.10 dB headroom. All renders are finite and unclipped with no browser errors. The approved player death reference is unchanged. Local production serves the exact replacement checksum. Formatting, lint, TypeScript, all 85 unit tests, and the production build pass. These are source and signal checks, not a perceptual listening assessment. Evidence: `/private/tmp/pdoom-boss-voice-replacement-audition/measurements.json`; [asset provenance and recipe](../public/game/audio/README.md#sam-boss-death-recording).
...
The social preview now uses an unaltered 1200 × 630 Chrome capture of the actual homepage menu, with the logo, subtitle, skull-selected Start action, and footer visible; no gameplay renderer is present. Identical 129,356-byte JPEGs live at `app/opengraph-image.jpg` and `app/twitter-image.jpg`, each with a native `.alt.txt` companion. The former `public/social-preview.jpg` and explicit image entries in layout metadata are removed. Next.js generates the image routes, cache-busting query strings, MIME/dimension/alt metadata, and absolute production URLs; the layout retains the canonical URL and Twitter `summary_large_image` card.

Production HTML checks with JavaScript disabled and Twitter/Facebook crawler user agents verify one corresponding image URL each, the correct production origin, alt text, Open Graph dimensions/type, and the large Twitter card. Both image endpoints return `image/jpeg` with bytes matching the source JPEGs, browser decoding confirms 1200 × 630, and the old asset returns 404. Formatting, lint, TypeScript, and the production build pass. Capture source and verification evidence: `/private/tmp/pdoom-home-social/capture.mjs` and `/private/tmp/pdoom-home-social/production-proof.json`.

The root layout continues to mount `@vercel/analytics/next` once. Earlier browser checks verified one deferred Analytics script and one queued pageview without browser errors; Analytics transport was isolated locally. Enable Web Analytics in the Vercel project when deploying and verify hosted delivery there. Evidence: `/private/tmp/pdoom-production-metadata-proof.json`.
